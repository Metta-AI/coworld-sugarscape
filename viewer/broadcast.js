...
function recordFrame(frame) {
  ...
  if (state.startingPopulation === 0) state.startingPopulation = frame.agents.length;
...
function buildTerrain(frame) {
  ...
  const hairline = Math.max(1, Math.round(cell * 0.05));
  terrainContext.fillStyle = GRID_HEX;
...
function interpolate(previous, frame, t) {
  ...
  return frame.agents.map((agent) => {
    ...
    return {
      ...
      wealth,
      // Less than one timestep of food left - it will not survive metabolism
      // unless it eats this move. At two timesteps the ring fired on roughly
      // seven times as many settlers as actually died.
...
function scorebug(frame) {
  ...
  markup += text(
    state.compact ? `most ${resourceName()} wins` : `two populations forage one mountain · most ${resourceName()} wins`,
...
function raceChart(frame, x, y, width, height) {
  ...
  // Scaled to the whole episode's peak, the trace was a flat line near zero for
  // the first half of every replay - a 615x225 panel holding 45px of ink. It now
  // scales to the largest gap SEEN SO FAR, which fills the plot from the first
  // timesteps. Taking the running maximum means it only ever grows, so the curve
  // never rescales downward under the viewer.
  const peak = Math.max(
    12,
  ...
  // Both poles printed a bare "+120", which made the scale unreadable. Each end
  // now names whose lead it measures.
  const axisEnd = (slot, baseline) => seatMark(left + 12, baseline - 5, slot, 6)
    + text(
      `${frame.slots[slot]?.name ?? `Population ${slot + 1}`} +${format(peak)}`,
      left + 26, baseline,
      { size: T(17), weight: 600, family: F.mono, fill: SEATS[slot].color, outline: 2.8 },
    );
  markup += axisEnd(0, top + 18);
  markup += axisEnd(1, top + plotH - 14);
  if (!state.compact) {
...
function emergence(frame, x, y, width, height) {
  ...
  const lost = state.startingPopulation - population;
  const rows = [
    ...
    ["Survivors", `${population}`,
      lost > 0 ? `of ${state.startingPopulation} · ${lost} starved` : `of ${state.startingPopulation}`],
  ];
...
function endCard(frame) {
  ...
  const context = `${survivors} of ${state.startingPopulation} settlers survived the mountain,`
    + ` and the richest of them ended up holding most of the ${resourceName()}.`;
