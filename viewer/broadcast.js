...
function buildTerrain(frame, from, clock, dissolved) {
  ...
  paintHeightWash(frame, cell);

  buildGrainSheet(cell);
  buildGrainDrift(frame.cells.length);
  const box = grainSheet.box;
  const bleed = grainSheet.bleed;
  const float = grainSheet.float;
  const ceiling = grainCeiling();
  const seconds = clock / 1000;
  const leaving = from && from.cells.length === frame.cells.length ? from.cells : null;
  let alpha = 1;
  terrainContext.globalAlpha = 1;

  for (let index = 0; index < frame.cells.length; index += 1) {
    const holds = frame.cells[index];
    const held = leaving ? leaving[index] : holds;
    if (holds[0] <= 0 && holds[1] <= 0 && held[0] <= 0 && held[1] <= 0) continue;
    ...
    // One margin up and to the left of the cell it belongs to, so the sand that
    // overhangs lands on the neighbours. Nothing is erased by the neighbour's own
    // blit: the margins are transparent, so the two fields simply overlap and the
    // boundary stops existing.
    const left = Math.round(x * cell) - bleed;
    const top = Math.round(y * cell) - bleed;
    for (let layer = 0; layer < GRAIN_LAYERS; layer += 1) {
      const slot = index * GRAIN_LAYERS + layer;
      // Sub-pixel, and deliberately not rounded: a fractional blit costs the same
      // as an integer one and is the whole reason this motion is continuous.
      const dx = left + float * Math.sin(seconds * grainDrift.rateX[slot] + grainDrift.phaseX[slot]);
      const dy = top + float * Math.sin(seconds * grainDrift.rateY[slot] + grainDrift.phaseY[slot]);
      for (let resource = 0; resource < 2; resource += 1) {
        const now = grainUnits(holds, resource, ceiling);
        const was = grainUnits(held, resource, ceiling);
        const settled = Math.min(now, was);
        if (settled > 0) {
          const tile = grainSolid(resource, settled, variant, layer);
          if (tile) {
            if (alpha !== 1) { alpha = 1; terrainContext.globalAlpha = 1; }
            terrainContext.drawImage(tile, dx, dy, box, box);
          }
        }
        if (now === was) continue;
        const rising = now > was;
        const top_ = Math.max(now, was);
        const queue = (top_ - settled) * GRAIN_SIPS;
        let place = 0;
        for (let unit = rising ? settled + 1 : top_; rising ? unit <= top_ : unit > settled;
          unit += rising ? 1 : -1) {
          for (let sip = 0; sip < GRAIN_SIPS; sip += 1) {
            const share = grainShare(dissolved, queue, place, rising);
            place += 1;
            if (share <= 0.004) continue;
            const tile = grainSipTile(resource, unit, sip, variant, layer);
            if (!tile) continue;
            if (alpha !== share) { alpha = share; terrainContext.globalAlpha = share; }
            terrainContext.drawImage(tile, dx, dy, box, box);
          }
        }
      }
...
function scorebug(frame) {
  ...
  rows.forEach((row, rank) => {
    ...
    const worthSaying = rank === 0 || rows.length > 2;
    if (worthSaying) {
      markup += text(
