 ...
 *
 * The phase a cell shows is offset by where that cell stands on the lattice. Two
 * cells holding the same amount already share a cloud whenever they draw the same
 * variant — one chance in eight — and if they also stood at the same phase they
 * would be pixel-identical twins, which is exactly the printed-repeat the variants
 * exist to break. The offset makes that one chance in eight times twenty-four.
 *
 ...
 * which moves every grain a fraction of a pixel fifteen times a second and never
 * stops, and the harvest, which is the resource actually changing hands. Held on
 * one paused timestep with the stir on, ~47% of the plate's pixels change every
 * 67ms step and 17% of them change by more than a quarter of the plate-to-grain
 * contrast; with it off, none do. So a viewer asked whether the board still
 * flickers cannot answer while both are running, and neither can an audit.
 *
...
function stirAt(now) {
  ...
  return Math.floor(now / (GRAIN_PERIOD_MS / GRAIN_PHASES));
}
...
/** Which frame of the loop the cell at (x, y) shows. Offset along x - y so that
 *  neighbours never stand at the same phase, and two cells that happen to share
 *  an amount AND a variant still cannot print as identical twins. */
  ...
  for (const [resource, colour] of [[0, SUGAR_HEX], [1, SPICE_HEX]]) {
    for (let units = 1; units <= ceiling[resource]; units += 1) {
      for (let variant = 0; variant < TILE_VARIANTS; variant += 1) {
        ...
        /* Every grain floats on a path of ITS OWN — its own start, its own
         * direction, its own ellipse at its own angle, none of it shared with
         * the grain beside it.
         *
         * The first version tied the starting angle to the grain's x so that
         * neighbours leaned together, on the theory that a wave passing through
         * the cell would look more like weather than independent orbits would.
         * It looks like neither. A cell whose grains all lean the same way is one
         * rigid thing tilting, and the plate read as several hundred little rafts
         * ROCKING back and forth in step. Correlation is the bug, not the fix:
         * with nothing shared, the mass simmers, and the cells mesh into one
         * field instead of each announcing its own edges.
         *
         * Precomputed per grain rather than per (grain, phase) — these are
         * constants of the path, and recomputing eight hashes inside the phase
         * loop cost twenty-four times what it needed to. */
        const paths = cloud.map(([x, y, scale], index) => {
          const tilt = grainOffset(stream, index, 5) * Math.PI;
          return {
            homeX: x * size,
            homeY: y * size,
            grade: particle * scale,
            spin: grainOffset(stream, index, 3) < 0.5 ? 1 : -1,
            start: grainOffset(stream, index, 4),
            cosTilt: Math.cos(tilt),
            sinTilt: Math.sin(tilt),
            reach: drift * (0.35 + grainOffset(stream, index, 6) * 0.65),
            // Round orbits all round is as uniform as flat ones all flat. Each
            // grain gets its own squash, so no two trace the same figure.
            flat: 0.3 + grainOffset(stream, index, 7) * 0.7,
          };
        });
