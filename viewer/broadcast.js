...
function drawBoard(frame, previous, t, now) {
  ...
  for (const body of bodies) {
    ...
    // SWAYED — this settler took a neighbour's culture on this timestep. Drawn
    // UNDER the body as a halo so it never hides the colour it just changed to,
    // which is the whole point of the event.
    if (body.swayed) {
      context.beginPath();
      context.arc(body.px, body.py, size + ring * 2.6, 0, Math.PI * 2);
...
function boardKey(frame) {
  // Sits on the baseline of its own row, immediately above the board.
  ...
  // The key has to teach what the board actually encodes, and it has been wrong
  // twice by lagging it: a colour ramp after the board became grain density, and
  // grain density after the board became countable dots. The swatches now draw
  // the SAME dots at the same sizes — none, one, two, three, four.
  ...
  for (const [label, grainColour, peak] of ramps) {
    ...
    for (let step = 0; step <= steps; step += 1) {
      ...
      // One dot per unit, laid out by the board's own sampler so the key is the
      // same picture a cell holding that much would print.
      const spot = Math.max(0.8, swatchW * DOT_RADIUS * 2.2);
      const layout = cellDotLayout(step + 900, 4, 4);
      for (let unit = 0; unit < step; unit += 1) {
        const point = layout[unit];
        if (!point) continue;
        markup += `<circle cx="${(cellX + point[0] * swatchW).toFixed(1)}" `
          + `cy="${(y - swatchW + 2 + point[1] * swatchW).toFixed(1)}" `
          + `r="${spot.toFixed(2)}" fill="${grainColour}"/>`;
      }
    }
  ...
  // When the world has cultures the settlers are coloured by TRIBE, so the key
  // has to teach tribes — a seat swatch here would name a colour the board never
  // draws. One swatch per surviving tribe, in the board's own inks.
  ...
  if (tribes.length > 0) {
    ...
    for (const tribe of tribes) {
      markup += `<circle cx="${x}" cy="${y - dot}" r="${dot}" `
        + `fill="${TRIBE_INK[tribe % TRIBE_INK.length]}"/>`;
      x += dot * 2 + G(5);
    }
    x += gap - G(5);
    markup += `<circle cx="${x}" cy="${y - dot}" r="${dot}" fill="none" `
      + `stroke="${C.paper}" stroke-width="${G(2.4)}" opacity="0.6"/>`;
    markup += text("just swayed", x + dot + G(7), y, style);
    x += dot + G(7) + advance("just swayed", size) + gap;
  }
  const budget = (rightEdge - x - tailWidth) / Math.max(1, frame.slots.length);
  frame.slots.forEach((slot, index) => {
    if (tribes.length > 0) return; // the seat is not what the body colour says
...
function targetHistogram(frame, x, y, width, height) {
  ...
  markup += text(`${seat.variable ?? "wealth"} · ${seat.targetId ?? "target"}`,
    x + 18, headY + T(40) + 44, { size: T(17), weight: 500, family: F.mono, fill: C.dim });
...
function inequalityOverTime(frame, x, y, width, height) {
  ...
  markup += text(now.toFixed(3), plotX, plotTop + plotH + 22,
    { size: T(22), weight: 600, family: F.mono, fill: C.paper });
  markup += text(way, plotX + advance(now.toFixed(3), T(22)) + 10, plotTop + plotH + 22,
    { size: T(17), weight: 500, family: F.mono, fill: C.dim });

  // Trade, as a footer — see the note above on what the replay does and does not
  // record. Zero is a real reading here, not a missing one.
  const stats = frame.stats ?? {};
  if (stats.tradeVolume !== undefined) {
    ...
    const line = volume === 0
      ? "no trades this tick"
      : `${volume} trade${volume === 1 ? "" : "s"} this tick · mean price ${price.toFixed(2)}`;
 ...
 */
const TRIBE_INK = ["#f0a63c", "#7fb3d5", "#c9d17a", "#d98cae", "#8fd1c0", "#e8c07d"];
