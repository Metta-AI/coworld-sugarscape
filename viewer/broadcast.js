...
function boardKey(frame) {
  ...
  frame.slots.forEach((slot, index) => {
    markup += `<circle cx="${x}" cy="${y - T(6)}" r="${T(6)}" fill="${SEATS[index].color}"/>`;
...
function scorebug(frame) {
  ...
  markup += text(
    state.compact
      ? `two populations · most ${resourceName()} wins`
      : `two populations forage one mountain · most ${resourceName()} wins`,
...
function raceChart(frame, x, y, width, height) {
    ...
    frame.slots.forEach((_, slot) => {
      ...
      markup += seatMark(left + 14, top + 16 + slot * 26, slot, 6)
        + text(frame.slots[slot]?.name ?? `Population ${slot + 1}`, left + 28, top + 21 + slot * 26,
          { size: T(17), weight: 600, fill: SEATS[slot].color, outline: 2.8 });
    });
  ...
  const flush = () => {
    ...
    markup += `<polygon points="${spell[0].x.toFixed(1)},${baseY} ${path} `
      + `${spell.at(-1).x.toFixed(1)},${baseY}" fill="${SEATS[spellLeader].color}" opacity=".42"/>`;
    markup += `<polyline points="${path}" fill="none" stroke="${SEATS[spellLeader].color}" `
  ...
  if (last) {
    ...
    markup += `<circle cx="${scaleX(last.timestep)}" cy="${headY}" r="6" `
      + `fill="${leader < 0 ? C.muted : SEATS[leader].color}" stroke="${C.ink}" stroke-width="1.8"/>`;
    markup += text(
      leader < 0 ? "level" : `${frame.slots[leader]?.name ?? `Population ${leader + 1}`} ahead`,
      left + 14, top + 20,
      { size: T(19), weight: 700, fill: leader < 0 ? C.muted : SEATS[leader].color, outline: 3 },
    );
  }
