...
def choose_ruleset(target: Mapping[str, object]) -> dict[str, object]:
    ...
    if target_id == "wealth.egalitarian":
        return {
            ...
            "movement": [{"score": ["-", welfare, ["*", 0.5, ["get", "cell.preyWealth"]]]}],
        }
...
def resolve_websocket_url(args: argparse.Namespace, environment: Mapping[str, str]) -> str:
    ...
    if args.slot is None or args.token is None:
        raise ValueError("COWORLD_PLAYER_WS_URL or both --slot and --token are required")
...
def run(url: str) -> None:
    ...
    with connect(url, max_size=64 * 1024, proxy=None) as websocket:
        ...
        websocket.send(json.dumps({"type": "action", "ruleset": ruleset}, separators=(",", ":")))
        acknowledgement = json.loads(websocket.recv())
        if acknowledgement.get("type") != "ack" or acknowledgement.get("valid") is not True:
            raise RuntimeError(f"game rejected bundled baseline ruleset: {acknowledgement}")


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Sugarscape target-aware baseline player")
