...
function drawBoard(frame, previous, t, now) {
  ...
  let cell = Math.min(BOARD.w / frame.width, BOARD.h / frame.height);
  ...
  easeView(frame, now);
  const lens = viewTransform(frame, cell);
  ...
  for (const body of bodies) {
    ...
    if (body.wrapped) {
      const dx = body.px < BOARD.x ? spanX : (body.px > BOARD.x + spanX ? -spanX : 0);
      const dy = body.py < BOARD.y ? spanY : (body.py > BOARD.y + spanY ? -spanY : 0);
      if (dx || dy) spots.push([body.px + dx, body.py + dy]);
      else {
        // Still inside the lattice, but about to leave it: draw the arrival
        // ahead of time on whichever axis the walk is crossing.
        spots.push([body.px - spanX, body.py], [body.px + spanX, body.py]);
      }
    }
